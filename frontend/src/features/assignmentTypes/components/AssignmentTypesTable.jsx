...
const columns = [
  { key: "ASSIGNMENT_TYPE", label: "Name", sortable: true },
...
const SKELETON_WIDTHS = ["60%", "75%", "45%", "55%"];
    ...
    <div className="flex flex-col gap-5 font-['Circular',_'Helvetica_Neue',_Helvetica,_Arial,_sans-serif]">
        ...
        <table className="w-full text-sm">
          ...
          <tbody className="divide-y divide-[#F7F7F7]">
            {loading ? (
              ...
              <tr>
                <td colSpan={columns.length + 1} className="px-5 py-16 text-center">
                  <div className="flex flex-col items-center gap-2">
                    ...
                    <p className="text-[#222222] font-semibold">No assignment types found</p>
              ...
              data.map((row) => (
                <tr
                  key={row.ASSIGNMENT_TYPE_ID ?? row.id}
                ...
                >
                  ...
                  <td className="px-5 py-4 text-[#717171] text-sm max-w-[300px] truncate">
                    {row.DESCRIPTION ?? "—"}
                  </td>
                  <td className="px-5 py-4">
      ...
      <div className="flex items-center justify-between text-sm text-[#717171]">
        <span className="text-xs">
          {resolvedTotal === 0
            ? "No results"
            : `${(page - 1) * limit + 1}–${Math.min(page * limit, resolvedTotal)} of ${resolvedTotal} assignment types`}
        </span>
